...
<div class="table-responsive" style="margin-top:100px;">
  <div class="table-wrapper" style="border-radius:15px;">
    ...
    <table class="table table-bordered table-stripe table-hover">
      ...
      <tbody>
        <?php $totalPrice = 0; ?>
        <?php foreach($data['mon_entities'] as $key => $mon_entity) :?>
        <tr id="<?php echo $mon_entity->id?>">
          <td><?php echo ($key+1);?></td>
          <td><?php echo $mon_entity->name; ?></td>
          <td><?php echo $mon_entity->price; ?></td>
          <td>
          ...
          <?php $totalPrice = $totalPrice + $mon_entity->price;?>
        </tr>
        <?php endforeach; ?>
        <tr>
          <td></td>
...
$(document).ready(function() {
  ...
  $(document).on("click", ".add", function() {
    ...
    if (!empty) {
      ...
      $.ajax({
        type: "POST",
        url: '<?php echo URLROOT; ?>/entities/monthly',
        data: {
          addData: tmpData
        },
